...
<body>
  ...
  <div class="contenu" style="clear:both; display:block; ">
    <span style="font-size: 110%;"><u>Entree du </u> : <b>{{$entree->created_at}}</b></span><br>
    <span style="font-size: 110%;"><u>Date de réception </u> : <b>{{$entree->date_reception}}</b></span><br>
    <span style="font-size: 110%;"><u>Référence </u> : <b>{{$entree->reference}}</b></span><br>
    <span style="font-size: 110%;"><u>Faire par </u> : <b>{{$entree->getUserCreated->name}}</b></span><br><br>
    @php
        $details = App\Models\DetailsEntrees::where('entrees_id', $entree->id)->get();
    @endphp
    <table style="width:100%;">
        ...
        @foreach ($details as $detail)
          ...
          @endforeach
          @php
          $prix_unitaire = DB::table('details_entrees')->where('entrees_id', $entree->id)->sum('details_entrees.prix_unitaire');
          $prix_total = DB::table('details_entrees')->where('entrees_id', $entree->id)->sum('details_entrees.prix_total');
          @endphp
          <tr>
            <td colspan="4" style="text-align: center;">Total</td>
            <td colspan="2"  style="text-align: right;">{{$prix_total}}</td>
          </tr>
        </tbody>
      </table>
  </div>
